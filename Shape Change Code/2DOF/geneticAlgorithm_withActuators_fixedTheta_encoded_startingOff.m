...
arduinoObj.angle = 0 + arduinoObj.thetaOff;
...
[P,popFitness,varFitnessVec] = mainLoop(N, Pc, Pm, SRange, LRange, RRange, nElite, threshold, arduinoObj);
xlabel('Number of loops for N=10');
ylabel('Fitness');

%% Main
function [P,popFitness,varFitnessVec] = mainLoop(N, Pc, Pm, SRange, LRange,RRange, nElite, threshold, arduinoObj)
...
for i = 1:10
popData(i,:) = dlmread(strcat('E:\Mark\Spring 2019\03182019_geneticAlgorithm\optimizeDrag\Population_17_Gene_',string(i),'_DNA'))
end
...
for i = 1:N
    ...
    dataFileName = strcat('Population_18_Gene_',num2str(i));
...
P_new = [P, ObjDrag+abs(min(ObjDrag))+0.02]; %Drag is +/- the offset value.
...
for i = 1:N
    ...
    avgForceVals = readLoadCell(i,P(i,3),arduinoObj,dataFileName,N);
    dnaFileName = strcat(fileLoc,dataFileName,'_DNA');
    dlmwrite(dnaFileName,[P(i,1:2), P(i,3), avgForceVals],'-append');
